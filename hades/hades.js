...
async function hades (todayDate){
  const sfpiId = [];
  const allData = [];
  // const today = dayjs.utc('2023-02-01T00:00:00').toDate();
  const today = dayjs.utc(todayDate).subtract(1, 'day').toDate();
  ...
  console.log(`${dayjs.utc(today).toISOString()} : data Generated`)
