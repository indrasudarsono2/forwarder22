...
module.exports = {
    ...
    try {
      ...
      sfpi.sort((a, b) => {
        ...
        if (aOvsetTime && bOvsetTime) {
          const timeDifference = Math.abs(new Date(aOvsetTime) - new Date(bOvsetTime));
          
          if (timeDifference < 2 * 60 * 1000 && a.clearedFlightLevel == b.clearedFlightLevel) { // 2 minutes in milliseconds
            a.traffic = [{"callsign" : b.callsign, "pointName" : a.path.find((point) => ['SOVMI', 'OVSET', 'JATAM'].includes(point.pointName)).pointName}];
            b.traffic = [{"callsign" : a.callsign, "pointName" : b.path.find((point) => ['SOVMI', 'OVSET', 'JATAM'].includes(point.pointName)).pointName}];
          }
  ...
  lebeg: async(req, res) => {
    ...
    try {
      ...
      sfpi.sort((a, b) => {
        const aLebegTime = a.path.find((point) => ['GUDBA', 'RIMTI', 'BIDOV'].includes(point.pointName))?.time;
        const bLebegTime = b.path.find((point) => ['GUDBA', 'RIMTI', 'BIDOV'].includes(point.pointName))?.time;

        if (aLebegTime && bLebegTime) {
          const timeDifference = Math.abs(new Date(aLebegTime) - new Date(bLebegTime));
          
          if (timeDifference < 2 * 60 * 1000 && a.clearedFlightLevel == b.clearedFlightLevel) { // 2 minutes in milliseconds
            a.traffic = [{"callsign" : b.callsign, "pointName" : a.path.find((point) => ['GUDBA', 'RIMTI', 'BIDOV'].includes(point.pointName)).pointName}];
            b.traffic = [{"callsign" : a.callsign, "pointName" : b.path.find((point) => ['GUDBA', 'RIMTI', 'BIDOV'].includes(point.pointName)).pointName}];
          }
          
          return new Date(aLebegTime).getTime() - new Date(bLebegTime).getTime();
        } else if (aLebegTime) {
          return -1;
        } else if (bLebegTime) {
          return 1;
    ...
    try {
      ...
      sfpi.sort((a, b) => {
        ...
        if (aBunikTime && bBunikTime) {
          ...
          if (timeDifference < 2 * 60 * 1000 && a.clearedFlightLevel == b.clearedFlightLevel) { // 2 minutes in milliseconds
            a.traffic = [{"callsign" : b.callsign, "pointName" : a.path.find((point) => ['BIKAL', 'BUNIK', 'IKIBU'].includes(point.pointName)).pointName}];
            b.traffic = [{"callsign" : a.callsign, "pointName" : b.path.find((point) => ['BIKAL', 'BUNIK', 'IKIBU'].includes(point.pointName)).pointName}];
          }
