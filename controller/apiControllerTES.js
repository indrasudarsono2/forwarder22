...
module.exports = {
  pardi: async(req, res) => {
    let now = new Date('2023-07-10T10:31:00Z');
  ...
  ovset: async(req, res) => {
    ...
    try {
      ...
      sfpi.sort((a, b) => {
        if (a.path.find((point) => point.pointName === "OVSET").time == null){
          var aOvsetTime = new Date(
            a.path.find((point) => point.pointName === "JATAM").time
          ).getTime();
        }else{
          var aOvsetTime = new Date(
        ...
        if(b.path.find((point) => point.pointName === "OVSET").time == null){
          var bOvsetTime = new Date(
            b.path.find((point) => point.pointName === "JATAM").time
          ).getTime();
        }else{
          var bOvsetTime = new Date(
  ...
  lebeg: async(req, res) => {
    ...
    try {
      ...
      sfpi.sort((a, b) => {
        if (a.path.find((point) => point.pointName === "GUDBA").time == null){
          var aTkgTime = new Date(
        ...
        if(b.path.find((point) => point.pointName === "GUDBA").time == null){
          var bTkgTime = new Date(
