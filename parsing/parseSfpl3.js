...
const { readData } = require('./parsingSector')
const dayjs = require('dayjs')
const utc = require('dayjs/plugin/utc');
const { matchFpl } = require('../aftn/matchingFpl');
dayjs.extend(utc);
...
async function processFile(sfpl) {
  try {
    const dataSfpl = sfpl;
    const fileSfpl = `REALTIME-SFPL`;
...
async function putDatabase(data){
  ...
  if(!file.path){
    ...
    if(!check){
      ...
      if(file.fieldsData.state == "TERMINATED"){
        ...
        const addDataInputed = (entryPoint, exitPoint, cat) => {
          dataInputed["dateProcess"] = dayjs.utc(entryPoint.time).toDate();
          dataInputed["pointEntry"] = entryPoint.pointName,
          dataInputed["pointExit"] = exitPoint.pointName,
          dataInputed["cat"] = cat
        }
        ...
        if(sfpi && sfpi.logPath.length != 0){
          ...
          if(entry || exit){
            if(!file.fieldsData.adep.startsWith('WI') && !file.fieldsData.adep.startsWith('WA') && !file.fieldsData.ades.startsWith('WI') && !file.fieldsData.ades.startsWith('WA')){
              addDataInputed(entry, exit, 'OVF')
            }else if((file.fieldsData.adep.startsWith('WI') || file.fieldsData.adep.startsWith('WA')) && file.fieldsData.ades != 'WIII' && file.fieldsData.adep != 'WIII' && file.fieldsData.ades != 'ZZZZ' && !file.fieldsData.ades.startsWith('WI') && !file.fieldsData.ades.startsWith('WA')){
              addDataInputed(entry, exit, 'OVF INT')
            }else if((file.fieldsData.adep.startsWith('WI') || file.fieldsData.adep.startsWith('WA')) && (file.fieldsData.ades.startsWith('WI') || file.fieldsData.ades.startsWith('WA')) && file.fieldsData.ades != 'WIII' && file.fieldsData.adep != 'WIII'){
              addDataInputed(entry, exit, 'OVF DOM')
            }
          }
        }
        dataInputed["dateInit"] = dayjs.utc(file.time).toDate();
      }else if(file.fieldsData.state == "ACTIVE" && file.fieldsData.adep == "WIII"){
        const check = await prisma.sfpis.findFirst({
          where: {
            id: String(file.fieldsData.sfpi),
            dateProcess: null
        ...
        if(check || check != null){
          dataInputed["dateProcess"] = dayjs.utc(file.fieldsData.estTimeOfEntry).toDate();
        }
      }else if(file.fieldsData.state == "ACTIVE" && file.fieldsData.ades == "WIII" && file.fieldsData.actualLandingTime != null){
        const check = await prisma.sfpis.findFirst({
          where: {
            id: String(file.fieldsData.sfpi),
            dateProcess: null
        ...
        if(check || check != null){
          dataInputed["dateProcess"] = dayjs.utc(file.fieldsData.actualLandingTime).toDate();
        }
      }
